...
test('balance 配置 JSON 可加载并通过 schema 校验', async () => {
  ...
  assert.equal(config.prodigy.almostAllProbability, 0.0005);
...
test('balance 配置校验拒绝非法载荷', () => {
  assert.throws(() => validateBalanceConfig({ schemaVersion: 2, rating: {}, prodigy: {} }), /schemaVersion/);
  assert.throws(() => validateBalanceConfig({ schemaVersion: 1, rating: { ...DEFAULT_BALANCE_CONFIG.rating, base: 'x' }, prodigy: DEFAULT_BALANCE_CONFIG.prodigy }), /rating\.base/);
  assert.throws(() => validateBalanceConfig({ schemaVersion: 1, rating: DEFAULT_BALANCE_CONFIG.rating, prodigy: { ...DEFAULT_BALANCE_CONFIG.prodigy, almostAllProbability: 0.9 } }), /almostAllProbability/);
  assert.throws(() => validateBalanceConfig({ schemaVersion: 1, rating: DEFAULT_BALANCE_CONFIG.rating, prodigy: { ...DEFAULT_BALANCE_CONFIG.prodigy, almostAllAttributes: ['teamConflict'] } }), /unknown attribute/);
});
